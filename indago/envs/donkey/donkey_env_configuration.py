...
class DonkeyEnvConfiguration(EnvConfiguration):
    ...
    def mutate_hot(self, attributions: np.ndarray, mapping: Dict) -> Optional["EnvConfiguration"]:
        ...
        if key_to_mutate == "commands":
            ...
            min_value, max_value = (1, 20) if track_element.command.name in ["S", "R", "L"] else (0.0, 50.0)
            track_element.value = self.polynomial_mutation(track_element.value, min_value, max_value, eta=20)
        else:
    ...
    def polynomial_mutation(value, lower_bound, upper_bound, eta=20):
        ...
        return int(round(mutated_value)) if isinstance(lower_bound, int) else round(mutated_value, 1)