...
def instantiate_trained_policy(
    ...
    exp_file: str = None,
    resume_dir: str = None,
    avf_train_policy: str = None,
    avf_test_policy: str = None,
    ...
    # NOTE: Removed this as it's fucking up my experiments!
    # if exp_id == 0:
    ...
    if enjoy_mode:
        ...
        if "replay_test_failure" in testing_strategy:
            testing_logs = os.path.join(log_path, f'replay_logs_{avf_train_policy}', avf_test_policy)
        else:
            testing_logs = os.path.join(log_path, f'testing_logs_{avf_train_policy}', avf_test_policy)
        os.makedirs(testing_logs, exist_ok=True)

        logging.basicConfig(
            filename=os.path.join(testing_logs, "testing-{}.txt".format(testing_strategy)), filemode="w", level=logging.DEBUG
        )

    if model_checkpoint != -1:
        model_path = os.path.join(log_path, "drl/model-checkpoint-{}.zip".format(model_checkpoint))
    else:
        model_path = os.path.join(log_path, "drl/best_model.zip")
