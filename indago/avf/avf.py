...
class Avf:
    def __init__(
        ...
        if exp_file is not None:
            ...
            file_split = exp_file.split('-')
            self.exp_file = os.path.join(self.log_path, f"testing_logs_{file_split[1]}/{file_split[2]}", exp_file)
    ...
    def sample_test_env_configuration(
        ...
        elif "pmga" in avf_test_policy:
            env_config, max_prediction = self.sample_test_env_configuration_pmga(
                avf_train_policy=avf_train_policy,
                avf_test_policy=avf_test_policy,
                population_size=population_size,
                crossover_rate=crossover_rate,
            )
        elif "nsga" in avf_test_policy:
            env_config, max_prediction = self.sample_test_env_configuration_pmnsga2(
                avf_train_policy=avf_train_policy,
                avf_test_policy=avf_test_policy,
                population_size=population_size,
                crossover_rate=crossover_rate,
            )
        elif "age" in avf_test_policy:
            env_config, max_prediction = self.sample_test_env_configuration_age(
                avf_train_policy=avf_train_policy,
                avf_test_policy=avf_test_policy,
                population_size=population_size,
                crossover_rate=crossover_rate,
    ...
    def sample_test_env_configuration_pmga(
        self, avf_train_policy: str, avf_test_policy: str, population_size: int, crossover_rate: float,
    ) -> Tuple[EnvConfiguration, float]:
        ...
        if "failure" in avf_test_policy:
            while len(pmga.initial_population) < len(self.failed_data_items) and len(pmga.initial_population) < population_size:
                idx_data = random.choices(population=np.arange(0, len(self.failed_data_items)), weights=self.weights_data)[0]
                while idx_data in self.indices_data_selected:
                    idx_data = random.choices(population=np.arange(0, len(self.failed_data_items)), weights=self.weights_data)[
                        0
                ...
                self.indices_data_selected.append(idx_data)
                failed_data_item = self.failed_data_items[idx_data]
                pmga.initial_population.append(Chromosome(env_config=failed_data_item.training_logs.get_config()))

        if "rnd" in avf_test_policy:
            while len(pmga.initial_population) < len(self.failed_data_items) and len(pmga.initial_population) < population_size:
                pmga.initial_population.append(Chromosome(env_config=self.generate_random_env_configuration()))

        # possible to introduce the same failures in the population across different runs
        self.indices_data_selected.clear()

        chromosome = pmga.run_mutations(num_generations=NUM_GENERATIONS, only_best=True, fitness_fn=fitness_fn, budget=self.budget)

        if chromosome is None:
            chromosome = pmga.population[0]
    ...
    def sample_test_env_configuration_pmnsga2(
        self, avf_train_policy: str, avf_test_policy: str, population_size: int, crossover_rate: float,
    ) -> Tuple[EnvConfiguration, float]:
        ...
        nsga2 = PMNSGA2(
            population_size=population_size,
            saved_environments=self.environments,
            generations=50,
            stopping_criterion_factory=stopping_criterion_factory,
            chromosome_factory=chromosome_factory,
            minimize=True,
        ...
        if "failure" in avf_test_policy:
            while len(nsga2.initial_population) < len(self.failed_data_items) and len(nsga2.initial_population) < population_size:
                idx_data = random.choices(population=np.arange(0, len(self.failed_data_items)), weights=self.weights_data)[0]
                while idx_data in self.indices_data_selected:
                    idx_data = random.choices(population=np.arange(0, len(self.failed_data_items)), weights=self.weights_data)[
                        0
                ...
                self.indices_data_selected.append(idx_data)
                failed_data_item = self.failed_data_items[idx_data]
                nsga2.initial_population.append(Chromosome(env_config=failed_data_item.training_logs.get_config()))
        
        if "rnd" in avf_test_policy:
            while len(nsga2.initial_population) < len(self.failed_data_items) and len(nsga2.initial_population) < population_size:
                nsga2.initial_population.append(Chromosome(env_config=self.generate_random_env_configuration()))

        # possible to introduce the same failures in the population across different runs
        self.indices_data_selected.clear()

        chromosome = nsga2.run_mutations(num_generations=NUM_GENERATIONS, only_best=True, fitness_fn=fitness_fn, budget=self.budget)

        if chromosome is None:
            chromosome = nsga2.population[0]
        ...
        self.logger.info("Env configuration: {}; max prediction: {}".format(self.current_env_config.get_str(), max_prediction))
        self.predictions.append(max_prediction)
        self.environments.append(self.current_env_config)

        return self.current_env_config, max_prediction

    def sample_test_env_configuration_age(
        self, avf_train_policy: str, avf_test_policy: str, population_size: int, crossover_rate: float,
    ) -> Tuple[EnvConfiguration, float]:
        ...
        if "failure" in avf_test_policy:
            while len(age.initial_population) < len(self.failed_data_items) and len(age.initial_population) < population_size:
                idx_data = random.choices(population=np.arange(0, len(self.failed_data_items)), weights=self.weights_data)[0]
                while idx_data in self.indices_data_selected:
                    idx_data = random.choices(population=np.arange(0, len(self.failed_data_items)), weights=self.weights_data)[
                        0
                ...
                self.indices_data_selected.append(idx_data)
                failed_data_item = self.failed_data_items[idx_data]
                age.initial_population.append(Chromosome(env_config=failed_data_item.training_logs.get_config()))
        
        if "rnd" in avf_test_policy:
            while len(age.initial_population) < len(self.failed_data_items) and len(age.initial_population) < population_size:
                age.initial_population.append(Chromosome(env_config=self.generate_random_env_configuration()))

        # possible to introduce the same failures in the population across different runs
        self.indices_data_selected.clear()

        chromosome = age.run_mutations(num_generations=NUM_GENERATIONS, only_best=True, fitness_fn=fitness_fn, budget=self.budget)

        if chromosome is None:
            chromosome = age.population[0]
    ...
    def store_testing_logs(self, training_logs: TrainingLogs) -> None:
        if not self.is_training:
            ...
            print(self.exp_file)
            if self.avf_test_policy == "replay_test_failure":
                testing_logs = os.path.join(self.log_path, f'replay_logs_{self.avf_train_policy}', self.avf_test_policy)
            else:
                testing_logs = os.path.join(self.log_path, f'testing_logs_{self.avf_train_policy}', self.avf_test_policy)
            os.makedirs(testing_logs, exist_ok=True)

            if self.num_trials != -1:
                filepath = os.path.join(testing_logs, self.testing_strategy_name, "trial-{}".format(self.num_trials))
                print(filepath)
                os.makedirs(filepath, exist_ok=True)
            else:
                filepath = os.path.join(testing_logs, self.testing_strategy_name)
                print(filepath)
                os.makedirs(filepath, exist_ok=True)
    ...
    def store_training_logs(self, training_logs: TrainingLogs) -> None:
        if self.storing_logs:
            file_num_last = 0
            log_path = os.path.join(self.log_path, "json_log/avf_log_{}_{}.json".format(self.file_num_first, file_num_last))
    ...
    def get_avf_save_path(
        ...
        surrogates = os.path.join(self.log_path, 'surrogates')
        ...
        if avf_train_policy in AVF_DNN_POLICIES:
            if training_progress_filter is not None:
                save_path = os.path.join(
                    surrogates,
                    f"best-avf-{avf_train_policy}-{training_progress_filter}-{oversample_minority_class_percentage}-{layers}.pkl",
                )
            else:
                save_path = os.path.join(
                    surrogates,
                    f"best-avf-{avf_train_policy}-{oversample_minority_class_percentage}-{layers}.pkl",
                )
