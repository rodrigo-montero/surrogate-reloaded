...
def sort_training_progress(env_name: str, log_path: str, filename: str) -> float:
    if filename.endswith(".json"):
        json_data = read_logs(log_path=f"{log_path}/json_log", filename=filename)
...
def preprocess_data(
    ...
    sorted_by_training_progress = sorted(
        os.listdir(path=f"{log_path}/json_log"), key=lambda f: sort_training_progress(env_name=env_name, filename=f, log_path=log_path)
    )
